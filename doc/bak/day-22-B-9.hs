...
union cu0 cu1 = go cu0 cu1 0
  where
    go cu0'@(cmd0,rng0) cu1'@(cmd1,rng1) idx
      | rng0 == rng1       = [cu0']        -- might need to change this if "off" ('-') cuboid
      | vMinCu1 > vMaxCu0  = [cu0', cu1']  -- non-overlapping
      | vMinCu0 < vMinCu1  = (cmd0,newRngA) : go (cmd0, newRngB) cu1' idx
      | vMinCu0 == vMinCu1 = [cu0', cu1'] -- ??? poss intersection ???
