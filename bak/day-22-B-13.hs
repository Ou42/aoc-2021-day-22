...
difference cu0@(cmd0, rngA) cu1@(cmd1, rngB) =
  ...
  in
    ...
    -- for mbZ, "update" rngA's yRng to be mbY's EQ
    error "nope!"
    -- mb -- [mbLT, mbEQ, mbGT]
    -- go rngA rngB 0
    --   where
...
intersects cu0@(cmd0, rngA) cu1@(cmd1, rngB) = go
  where
    go = foldr inside False rngAB
    rngAB = zip rngA rngB
