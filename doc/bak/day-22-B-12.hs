...
union cu0@(cmd0, rngA) cu1@(cmd1, rngB) =
  let
    mb   = myBreak (head rngA) (head rngB)
    -- mbLT = (head . head $ mb, tail . head $ mb)
    -- mbEQ = (head $ (!!) mb 1, tail $ (!!) mb 1)
    -- mbGT = (head $ (!!) mb 2, tail $ (!!) mb 2)
  in mb -- [mbLT, mbEQ, mbGT]
    -- go rngA rngB 0
    --   where
