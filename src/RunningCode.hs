...
-- ( sort, sortBy, groupBy, intersect, intersperse, intercalate, tails )
...
inputTest2 :: [Char]
inputTest2 = "data/Day-22-INPUT-test2.txt"
inputReal :: [Char]
...
type Rng a      = (a,a)
type Cuboid a   = [Rng a]
-- newtype Cuboid a   = Cuboid [Rng a]

-- 'PwrStep4 a' doesn't req {-# LANGUAGE RankNTypes #-}

type PwrStep4 a = (Char, Cuboid a, XYZ, LWH)
type PwrStep5 a = (Char, Cuboid a)
-- newtype PwrStep5 a = PwrStep5 (Char, Cuboid a)          -- refactor step 1
-- data PwrStep5 a = PwrStep5 { action :: Char
--                            , cuboid :: Cuboid a }    -- refactor step 2
...
intersectUsingSets3 :: (Num a, Ord a, Enum a) => Cuboid a -> Cuboid a -> Maybe (Cuboid a)
intersectUsingSets3 cuA@[ax,ay,az] cuB@[bx,by,bz] =
  let ixs = S.intersection (S.fromList [fst ax..snd ax]) (S.fromList [fst bx..snd bx])
      iys = S.intersection (S.fromList [fst ay..snd ay]) (S.fromList [fst by..snd by])
      izs = S.intersection (S.fromList [fst az..snd az]) (S.fromList [fst bz..snd bz])
      hasIntersection = not $ null ixs || null iys || null izs
      ...
      nys = (S.findMin iys, S.findMax iys)
      nzs = (S.findMin izs, S.findMax izs)
  in if hasIntersection then Just [nxs, nys, nzs] else Nothing
intersectUsingSets3 _ _ = error "Wrong # of ranges!"

-- diffUsingSets :: (Num a, Ord a, Enum a) => [Rng a] -> Rng a[] -> [[Rng a]]
diffUsingSets :: (Num a, Ord a, Enum a) => Cuboid a -> Cuboid a -> [Cuboid a]
-- diffUsingSets4Debug cmd opCuboid targetCuboid
diffUsingSets opCuboid@[] targetCuboid = []
diffUsingSets opCuboid targetCuboid@[(txMn,txMx),(tyMn,tyMx),(tzMn,tzMx)] =
  let
      -- ... the following works due to LAZY EVALUATION ...
      -- (hasIntersection, intersection) = intersectUsingSets2 opCuboid targetCuboid
      -- 04-08 per OFD instead of (Bool, Cuboid a) return Maybe (Cuboid a)
      -- intersection = intersectUsingSets3 opCuboid targetCuboid
      intersection = intersects4 opCuboid targetCuboid

      -- [iax@(iaxMn, iaxMx),iay@(iayMn, iayMx),iaz@(iazMn, iazMx)] = intersection
      Just [iax@(iaxMn, iaxMx),iay@(iayMn, iayMx),iaz@(iazMn, iazMx)] = intersection

      dxsLT = if txMn  < iaxMn then [(txMn,iaxMn-1), (tyMn,tyMx),    (tzMn,tzMx)] else []
      dxsGT = if iaxMx < txMx  then [(iaxMx+1,txMx), (tyMn,tyMx),    (tzMn,tzMx)] else []
      dysLT = if tyMn  < iayMn then [iax,            (tyMn,iayMn-1), (tzMn,tzMx)] else []
      dysGT = if iayMx < tyMx  then [iax,            (iayMx+1,tyMx), (tzMn,tzMx)] else []
      dzsLT = if tzMn  < iazMn then [iax,            iay,         (tzMn,iazMn-1)] else []
      dzsGT = if iazMx < tzMx  then [iax,            iay,         (iazMx+1,tzMx)] else []
      diffs = filter (not . null) [dxsLT, dxsGT, dysLT, dysGT, dzsLT, dzsGT]

  -- in if hasIntersection then diffs else [] <---- is THIS the BUG?! diff op B == B when no intersection?!
  -- in if hasIntersection then diffs else [targetCuboid]
  in case intersection of
       Just _ -> diffs
       _      -> [targetCuboid]
diffUsingSets _ _ = error "Wrong # of ranges!"

-- diffUsingSets2 cuA cuB = diffUsingSets cuB cuA

diffUsingSets4Debug cmd opCuboid targetCuboid  = -- diffUsingSets cuB cuA
  let diff = diffUsingSets targetCuboid opCuboid
      chkV = chkSameVol diff
      chkU = chkUnion cmd diff opCuboid targetCuboid
  -- in diff
  in case cmd of
      '+' -> if fst chkU then diff else error $ show chkU
      -- '+' -> if fst chkV then diff else error $ show chkV
      '-' -> diff
      -- '-' -> if fst chkV then diff else error $ show chkV
      _   -> error "Bad 'cmd': must be '+' or '-'"

chkSameVol cuLst =
-- ((solveA file) == (fst $ solveAv3 as)) || ((solveA file) == (snd $ solveAv3 as))
  let pwrStepLst = map ((,) '+') cuLst
      sAv3@(solveAv3Fst,solveAv3Snd) = solveAv3 pwrStepLst
      diffVol = vol cuLst
      sAv3Lst = [solveAv3Fst,solveAv3Snd]
  in (diffVol `elem` sAv3Lst, ((diffVol, sAv3Lst), cuLst))

-- chkUnion :: (Num a, Ord a, Enum a) => cmd -> [Cuboid a] -> Cuboid a -> Cuboid a -> Bool
-- chkUnion :: p -> [[(Int, Int)]] -> [[(Int, Int)]] -> [[(Int, Int)]] -> Bool
chkUnion cmd remnants cuA cuB = -- True
  let pwrStepLst = map ((,) '+') remnants
      (hasIntersection, intersection) = intersectUsingSets2 cuA cuB
      volIntersection = if hasIntersection then vol [intersection] else 0
      volRemnants     = vol remnants
      volA            = vol [cuA]
      volB            = vol [cuB]
      res             = (volA + volB - volIntersection) == (volRemnants + volA)
  in (res, (cuA, cuB))
...
-- mk3 file =
--   let do d <- readFile file
--   in (d, allSteps d, map snd $ allSteps d)

-- +-------------------------------------------------+
-- | the following requires: import System.IO.Unsafe |
-- +-------------------------------------------------+
file :: String
file = unsafePerformIO $ readFile i3


-- as :: (Num a) => String -> [PwrStep5 a] -- [(Char, [(Int, Int)])]
as   = allSteps file
sas = map snd as

-- pp == pretty print
-- pp (n, cuboid) = putStrLn $ printf "(% 3d, [" n ++ go cuboid ++ "])"
pp (n, cuboids) = printf "(% 3d, %s)" n (ppCuboids cuboids)

ppCuboid cuboid = "[" ++ go cuboid ++ "]"
  where
    -- go cuboid' = concatMap (\(f,s) -> printf "(% 3d,% 3d)" f s) cuboid'
    -- go cuboid' = concatMap (uncurry (printf "(% 3d,% 3d)")) cuboid'
    -- go cuboid' = L.concat $ L.intersperse "," $ map (uncurry (printf "(% 3d,% 3d)")) cuboid'
    go cuboid' = L.intercalate "," (map (uncurry (printf "(% 3d,% 3d)")) cuboid')

ppCuboids cuboids = L.intercalate "," (map ppCuboid cuboids)

-- let d = readFile i3

-- processCuboids cuLst = foldl (\accu cu -> (cu : [head accu]) ++ accu) [[]] cuLst
processPwrSteps stepLst = foldl go [(0,[])] stepLst -- FTW!!!
  where
    go accu@(h:t) step = case h of
      (_,[])    -> (1,[snd step]) : accu
      (i,lst)   -> (succ i, snd step : lst) : accu
    go _ _ = error "Pattern unexpected!"

processPwrSteps2 stepLst = foldl go [(0,[])] stepLst -- FTW!!!
  where
    go accu@(h:t) step@(cmd, cuboid) = case h of
      (_,[])    -> (1,[cuboid]) : accu
      (i,lst)   -> (succ i, cuboid : lst) : accu
    go _ _ = error "Pattern unexpected!"

{-
processPwrSteps3 stepLst = foldl go [(0,[])] stepLst -- FTW!!!
  where
    go accu@(h:t) step@(cmd, cuboid) = case h of
      (_,[])    -> (1,[cuboid]) : accu
--      (i,lst)   -> (succ i, cuboid : lst) : accu
      (i,lst)   -> (succ i, cuboid : concatMap (diffUsingSets2 cuboid) lst) : accu
--      concatMap (\cuA -> diffUsingSets cuA cuB) cuLst
    go _ _ = error "Pattern unexpected!"
-}

processPwrSteps4 stepLst = foldl go [(0,[])] stepLst -- FTW!!!
  where
    go accu@(h:t) step@(cmd, opCuboid) = case (cmd,h) of
      ('+',(i,[]))    -> (succ i,[opCuboid]) : accu
      ('-',(i,[]))    -> (succ i,[]) : accu
--      (i,lst)   -> (succ i, opCuboid : lst) : accu
      ('+',(i,lst))   -> (succ i, (opCuboid : concatMap (diffUsingSets opCuboid) lst)) : accu
      ('-',(i,lst))   -> (succ i,             concatMap (diffUsingSets opCuboid) lst)  : accu
      -- ('+',(i,lst))   -> (succ i, (opCuboid : concatMap (diffUsingSets4Debug cmd cuboid) lst)) : accu
      -- ('-',(i,lst))   -> (succ i,           concatMap (diffUsingSets4Debug cmd cuboid) lst)  : accu
      (_,_)           -> error "Your program go BOOM!"

    go _ _ = error "Pattern unexpected!"

iFreeProbs cuLst = foldl (\accu (cu:cus) -> (map (\cu2 -> if (intersectUsingSets cu cu2)
                                                            then [42]
                                                            else []) cus):accu) [] ((init . L.tails) cuLst)

iFreeProbs2 cuLst = foldl (\accu (cu:cus) -> (map (\cu2 -> if (intersectUsingSets cu cu2)
                                                            then [cu,cu2]
                                                            else []) cus):accu) [] ((init . L.tails) cuLst)

iFreeAllLsts cuLst = foldl (\accu (cu:cus) -> (map (intersectUsingSets cu) cus):accu) [] ((init . L.tails) cuLst)
...
sortOnXmin = L.sortBy (compare `on` this) -- (flip compare ...) sorts in descending order
  where
    this (_,here) = fst $ head here
--     this (PwrStep5 (_,here)) = fst $ head here

sortXYZs :: Ord a => Int -> [[(a,a)]] -> [[(a,a)]]
sortXYZs 3   = error "Index == 3. Can't sort!"
sortXYZs idx = L.sortBy (compare `on` (fst . (!! idx))) -- (flip compare ...) sorts in descending order
...
solveAv2 fileData =
  let -- lns      = lines fileData
      -- steps    = map (keepFstAltSnd . tuplify2 . words) lns
      steps    = allSteps fileData
      pm50     = filter keepPlusMinus50 steps
      cubesLst = map buildCubesList pm50
  in
      length $ foldl setify S.empty cubesLst

-- solveAv3 :: (Ord a1, Num a1, Enum a1) => [PwrStep5 a] -> (Int,Int)
solveAv3 steps =
  let -- lns      = lines fileData
      -- steps    = map (keepFstAltSnd . tuplify2 . words) lns
      -- steps    = allSteps fileData
      absLT51 = filter keepPlusMinus50 steps
      cuLst51 = map buildCubesList absLT51
      cuLst   = map buildCubesList steps
  in
      ( length $ foldl setify S.empty cuLst51
      , length $ foldl setify S.empty cuLst)
...
-- allSteps :: String -> [PwrStep5 a] -- [(Char, [(Int, Int)])]
allSteps fileData = steps
  where
    lns   = lines fileData
    -- steps :: [String] -> [PwrStep5 a]
    steps = map (keepFstAltSnd . tuplify2 . words) lns
...
vol :: [[(Int, Int)]] -> Int
vol [[]]  = 0
vol cuLst = sum $ map (product . map (\(mn,mx) -> abs(mx-mn)+1)) cuLst
...
verts d =
  ...
  $ map ( (map ((L.intersperse " ") . (:)"v")) . lst2Tup2Lst3 . snd ) $ allSteps d
...
-- intersects4 :: (Num a, Ord a, Enum a) => PwrStep5 a -> PwrStep5 a -> Bool
intersects4 cuA cuB = go
  where
    -- go = all (==True) chkXYZ
    go = if chkXYZ then Just xyzMxMn else Nothing
    rngAB  = zip cuA cuB
    -- chkXYZ  = all (==True) [  s1 `elem` [s2..e2] || e1 `elem` [s2..e2]
    --          || s2 `elem` [s1..e1] || e2 `elem` [s1..e1] | ((s1,e1),(s2,e2)) <- rngAB]
    chkXYZ  = all (==True) [  (s1 >= s2 && s1 <= e2) || (e1 >= s2 && e1 <= e2)
                           || (s2 >= s1 && s2 <= e1) || (e2 >= s1 && e2 <= e1)
                           | ((s1,e1),(s2,e2)) <- rngAB]

    xyzMxMn = [  (max s1 s2, min e1 e2) | ((s1,e1),(s2,e2)) <- rngAB]

{-
            (max, min) work                           (max, min) does NOT work

  s1, e1: xxxx    xxxx    xxxx      xxxx   xx   | xxxx             xxxx
  s2, e2: xxxx   xxxx       xxxx     xx   xxxx  |      xxxx   xxxx
